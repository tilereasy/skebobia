...
def act(
    ...
    selector = tick + sum(ord(ch) for ch in agent.id)
    ...
    if goal == GOAL_RESPOND and context.direct_messages:
        ...
        if source_type == "agent" and source_id and agent.message_cooldown == 0:
            ...
            return ActionIntent(
                kind="message",
                text=text,
                tags=["agent_message", "dialogue"],
                target_id=source_id,
            )
        text = templates.render("respond_user", selector)
        return ActionIntent(kind="say", text=text, tags=["dialogue", "user_message"])
    ...
    if goal == GOAL_HELP:
        ...
        if agent.say_cooldown == 0:
            ...
            return ActionIntent(kind="say", text=text, tags=["dialogue", "help"])
    ...
    if goal == GOAL_COMFORT:
        ...
        if agent.say_cooldown == 0:
            ...
            return ActionIntent(kind="say", text=text, tags=["dialogue", "memory"])
    ...
    if goal == GOAL_PANIC:
        if agent.say_cooldown == 0 and traits.aggression < 65:
            text = templates.render("panic", selector)
            return ActionIntent(kind="say", text=text, tags=["dialogue", "conflict"])
        return ActionIntent(kind="move", destination=SAFE_POINT)
    ...
    if context.emit_memory_event and agent.say_cooldown == 0:
        ...
        return ActionIntent(kind="say", text=text, tags=["memory"])

    if agent.say_cooldown == 0 and tick % 5 == 0 and traits.sociability > 45:
        text = templates.render("explore", selector)
        return ActionIntent(kind="say", text=text, tags=["dialogue"])
