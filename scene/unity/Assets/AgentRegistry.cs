...
public sealed class AgentRegistry : MonoBehaviour
{
    ...
    public void CreateOrUpdate(IReadOnlyList<AgentState> agents)
    {
        ...
        {
            string missingId = missingIds[i];
            agentObjects.Remove(missingId);
            agentViews.Remove(missingId);
            agentRenderers.Remove(missingId);
            agentLabels.Remove(missingId);
        }
    }
    ...
    public bool ShowBubble(string agentId, string text, float ttlSec)
    {
        ...
        if (!agentObjects.TryGetValue(agentId, out GameObject agentObject) || agentObject == null)
        {
            return false;
        }
    ...
    private GameObject CreateAgentObject(string agentId)
    {
        ...
        if (agentPrefab != null)
        {
            go = Instantiate(agentPrefab, agentsRoot);
        }
        else
        {
            go = new GameObject($"Agent_{agentId}");
            go.transform.SetParent(agentsRoot, false);
        }

        go.name = $"Agent_{agentId}";
